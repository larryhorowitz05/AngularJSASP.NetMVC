...
{
	public class AlertGridSortingCommand : ICommand
	{
		...
		public void Execute()
		{
			...
			AlertsAttribute newSortColumn;
			if ( !InputParameters.ContainsKey( "Column" ) )
				throw new ArgumentException( "Column value was expected!" );
			else
				newSortColumn = ( AlertsAttribute )Enum.Parse( typeof( AlertsAttribute ), InputParameters[ "Column" ].ToString() );

			if ( alertListState.SortColumn == newSortColumn && alertListState.SortDirection == "ASC" )
			{
				alertListState.SortDirection = "DESC";
			}
			else
				alertListState.SortDirection = "ASC";

			alertListState.SortColumn = newSortColumn;
