module util {
    ...
    export class addressWidget implements ng.IDirective {
        ...
        constructor(ZipCodeSvc) {
            ...
            this.controller = ($scope) => {
                ...
                if ($scope.stateName)
                    this.SetStateId($scope);

                if (!$scope.contentUrl)
                    $scope.contentUrl = '/angular/common/address/address.html';

                $scope.fetchZipData = () => {
                    return this.fetchZipData($scope, ZipCodeSvc);
                }

                $scope.isExistingState = (stateName) => {
                    return this.isExistingState($scope, stateName);
                }

                $scope.invalidZipCode = false;
                $scope.isLicencedZipCode = true;
                $scope.validateZipOnBlur = () => {
            ...
            this.compile = (element, attrs) => {
                ...
                if (!attrs.placeholderStateName) { attrs.placeholderStateName = 'State'; }
        ...
        private isExistingState = ($scope, stateName) => {
            for (var i = 0; i < $scope.states.length; i++)
                if ($scope.states[i].text == stateName) return true;
            return false;
        }

        private updatePropertyAddressFromZipData = ($scope, zipData) => {
            console.log("It is required ::: " + $scope.required);

            if (zipData.zipCode == null || !this.isExistingState($scope, zipData.stateName)) {
                $scope.cityName = "";
                $scope.countyName = "";
                $scope.stateName = "";
                $scope.zipCode = "";
        ...
        private fetchZipData = ($scope, ZipCodeSvc) => {
            ...
            ZipCodeSvc.GetZipData(payload).then((data) => {
                this.updatePropertyAddressFromZipData($scope, data);
            });
        };
        ...
        **/
        ...
        private validateZipOnBlur = ($scope) => {
            if ($scope.zipBlurValidation) {
