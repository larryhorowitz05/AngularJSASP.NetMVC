	  ...
	  .directive('modalpopoverWindow', ['$modalpopoverStack', '$timeout', '$document', function ($modalpopoverStack, $timeout, $document) {
	  	...
	  	return {
	  	    ...
	  	    scope: {
                ...
                calculateVerticalPositionFromTopBorder: '@'
	  	    },
	  	    replace: true,
	  	    transclude: true,
	  	    ...
	  	    link: function (scope, element, attrs) {
	  	        ...
	  	        $timeout(function () {
	  	            ...
	  	            if (scope.calculateVerticalPositionFromTopBorder == 'true') {
	  	                scope.modalPosition.top = parseFloat(scope.top) - (parseFloat(element[0].offsetHeight) * scope.verticalPopupPositionPerHeight);
	  	                scope.modalPosition.arrowPosition = parseFloat(element[0].offsetHeight) - 5;
	  	            } if (!angular.isUndefined(scope.position) && scope.position == 'fixed') {
	  	                //modal popover top position for readonly mode
	  	                scope.modalPosition.top = 27;
	  	            } else {
	  	            ...
	  	            scope.modalPosition.arrowPosition += 'px';
	  	            scope.modalPosition.zindex = (!angular.isUndefined(scope.zIndex) && parseFloat(scope.zIndex) > 0 ? parseFloat(scope.zIndex) : 1080) + parseFloat(scope.index) * 10;
	  	            scope.modalPosition.position = angular.isUndefined(scope.position) ? 'absolute' : scope.position;
	  	        });

                // Closes the popover if clicked elsewhere outside.
	  	        function onDocumentClick(event) {
	  	            var isChild = $(element).find(event.target).length > 0;
	  	            var isOnDocument = $(document).find(event.target).length > 0;

	  	            if (!isChild && isOnDocument && (scope.isOpen))
	  	                scope.close();
	  	        };
		...
		function ($transition, $timeout, $document, $compile, $rootScope, $$stackedMap) {
			...
			$modalpopoverStack.open = function (modalInstance, modal) {
				...
				angularDomEl.attr({
					...
					'calculate-vertical-position-from-top-border': modal.calculateVerticalPositionFromTopBorder,
				}).html(modal.content);
