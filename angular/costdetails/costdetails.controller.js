...
(function () {
    ...
    costDetailsController.$inject = ['$q', '$state', '$filter', '$rootScope', 'pricingResultsSvc', 'BroadcastSvc', 'commonService', 'costDetailsHelpers', 'costDetailsSvc', 'enums', 'NavigationSvc', 'wrappedLoan', 'simpleModalWindowFactory', 'applicationData', '$window', 'modalPopoverFactory', '$controller', '$scope', 'commonintegrationsSvc', '$http', 'blockUI', 'signalrFactory', 'loanEvent', 'vaCenterService', '$stateParams', 'ImpoundCalculatorHelpersSvc'];
    function costDetailsController($q, $state, $filter, $rootScope, pricingResultsSvc, BroadcastSvc, commonService, costDetailsHelpers, costDetailsSvc, enums, NavigationSvc, wrappedLoan, simpleModalWindowFactory, applicationData, $window, modalPopoverFactory, $controller, $scope, commonintegrationsSvc, $http, blockUI, signalrFactory, loanEvent, vaCenterService, $stateParams, ImpoundCalculatorHelpersSvc) {
        var vm = this;
        ...
        function getCostDetailsData(costsRecalculated) {
            if (costsRecalculated === void 0) { costsRecalculated = false; }
        ...
        function checkVaRules() {
            if (vm.wrappedLoan.ref.vaInformation && vm.wrappedLoan.ref.vaInformation.isvaLoan && !$stateParams.redirectedFromLoanDetailsVaAlertModal) {
                vaCenterService.isVAFeeLimitExceeded(vm.wrappedLoan.ref.closingCost, vm.wrappedLoan.ref.detailsOfTransaction.totalLoanAmount).$promise.then(function (data) {
                    ...
                    vm.wrappedLoan.ref.updateGovermentEligibility(4 /* VA */, !data.vaFeeLimitExceededResults.length);
                }, function (error) {
                    console.log(error);
                });
            }
        ...
        function onRightSectionAmountBlur(cost, model, rightSectionPaidType) {
            ...
            if (cost.templateId == 5 || cost.templateId == 4) {
                ...
                var atClose = cost.upfrontPreferredPaymentPeriod == 1 /* Financed */;
        ...
        function addCost(useLoanCostsList, section) {
            ...
            newItem.serviceProvider = 0 /* BorrowerSelected */;
        ...
        function zeroItemsFilter(item) {
            ...
            return item.editMode == true || flagReserves || item.borrowerPaid.atClosing != 0 || item.borrowerPaid.beforeClosing != 0 || item.sellerPaid.atClosing != 0 || item.sellerPaid.beforeClosing != 0 || item.otherPaid.atClosing != 0 || item.otherPaid.beforeClosing != 0 || item.amountMethod == 1 /* Itemized */;
        }
        ...
        function refreshSmartGFE() {
            pricingResultsSvc.RefreshSmartGFE.Refresh(vm.wrappedLoan.ref).$promise.then(function (data) {
        ...
        function createSmartGFE() {
            ...
            var createGFEResult = pricingResultsSvc.CreateSmartGFE(vm.wrappedLoan.ref).then(function (data) {
            ...
            var getToken = commonintegrationsSvc.services.getClosingCorpSSOToken({ path: 'GetClosingCorpSSOToken' }, {}, function () {
        ...
        function showLoanEstimateFeePopup(model, event) {
            ...
            modalPopoverFactory.openModalPopover('angular/costdetails/loanestimatefees.html', ctrl, initialModel, event).result.then(function (data) {
        ...
        function serviceProviderChanged(model) {
            ...
            if (model.serviceProvider == 1 /* Affiliate */)
                model.toleranceLevel = 0;
            else if (model.serviceProvider == 2 /* SSP */)
        ...
        function disableEstimateFeeTolerance(model) {
            return model.serviceProvider == 1 /* Affiliate */;
        }
        ...
        function openImpoundCalculator(event) {
            ...
            var impoundCalculator = modalPopoverFactory.openModalPopover('angular/common/impoundcalculator/impoundcalculator.html', $controller("impoundCalculatorController", { $scope: $scope.$new() }), {
                ...
                HOAExpens: { preferredPayPeriod: 1 /* Monthly */, amountPerMonth: vm.wrappedLoan.ref.housingExpenses.newHoa, amount: vm.wrappedLoan.ref.housingExpenses.newHoa },
            ...
            }, event, { className: 'tooltip-arrow-right-cost-details', verticalPopupPositionPerHeight: 0, horisontalPopupPositionPerWidth: 0.695 });
            impoundCalculator.result.then(function (data) {
                ...
                loanEvent.broadcastPropertyChangedEvent(28 /* AggregateAdjustment */, wrappedLoan.ref.totalAggregateAdjustment);
            }, function () {
            });
        }
        function openItemizedPropertyTaxCalculator(event, cost) {
            var itemizedPropertyTaxCalculator = modalPopoverFactory.openModalPopover('angular/common/itemizedpropertytaxcalculator/itemizedpropertytaxcalculator.html', $controller("itemizedPropertyTaxCalculatorController", {
                paymentPeriodMonths: vm.applicationData.lookup.periodPaymentMonths
            }), cost, event, {});
            itemizedPropertyTaxCalculator.result.then(function (data) {
                var propertyTax = _.where(wrappedLoan.ref.closingCost.costs, { hudLineNumber: 1004 })[0];
        ...
        function isItemizeCalculatorVisible(loanCost) {
            return loanCost.hudLineNumber == 1004 && loanCost.amountMethod == 1 /* Itemized */;
        }
        ...
        function recalculateAggregateAdjustment(loanCost) {
            if (loanCost.hudLineNumber == 1002 || loanCost.hudLineNumber == 1003 || loanCost.hudLineNumber == 1004 || loanCost.hudLineNumber == 1006 || (loanCost.isRemoved != true && costDetailsHelpers.isOtherInsurance(loanCost)))
                loanEvent.broadcastPropertyChangedEvent(28 /* AggregateAdjustment */, wrappedLoan.ref.totalAggregateAdjustment);
        }
...
//# sourceMappingURL=costdetails.controller.js.map