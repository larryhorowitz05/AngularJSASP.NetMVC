...
(function () {
    ...
    function costDetailsHelpers(costDetailsSvc, enums, commonService, $filter, BroadcastSvc, $state) {
        var service = {
        ...
        * Method definitions
        */
        function initializeCostService(wrappedLoan, applicationData) {
        ...
        function payoffCommentChanged(payoffItem) {
            ...
            var isPledgedAssetPayOff = payoffItem.isPledged && (payoffItem.payoffCommentId != enums.PledgedAssetComment.PayoffAtClose && payoffItem.payoffCommentId != enums.PledgedAssetComment.PayoffAtClosingAndDontCloseAccount && payoffItem.payoffCommentId != enums.PledgedAssetComment.PayoffAtClosingAndCloseAccount);
        ...
        function populatePledgedAssetComments(pledgetAssetComments) {
            ...
            var commentNotMyLoan = _.find(pledgetAssetComments, function (comment) {
                return comment.value === enums.PledgedAssetComment.NotMyLoan.toString();
            });
            if (commentNotMyLoan) {
        ...
        function calculateLoanOrOtherCostsTotals(allTotals, costs, lenderCreditsCosts, skipClosingCostTotalsCalculation, isOtherSection) {
            ...
            angular.forEach(costs, function (cost) {
                if (cost.isRemoved || (isOtherSection && !cost.impounded && cost.sectionId == 6 /* InitialEscowPaymentAtClosing */)) {
                    return true;
                }
        ...
        function AddPayoffItems(payoffs) {
            var totalAmount = 0;
        ...
        function getCostsTotalByHUDLineNumber(hudLineNumber, subHudLineNuumber) {
            var total = 0;
            lib.forEach(costDetailsSvc.wrappedLoan.ref.closingCost.costs, function (cost) {
        ...
        function getCostTotalAmount(cost) {
        ...
        function recalculateRightSectionAmounts(cost, model, rightSectionPaidType, borrowerPaidMax, atClose) {
            if (atClose === void 0) { atClose = true; }
        ...
        function setBorrowerPaidAmount(borrowerPaid, amount, atClose) {
            if (!atClose)
        ...
        function diferenceBetweenSumAndMax(cost, borrowerPaidMax, atClose) {
        ...
        function calculateBorrowerPTCAmount(cost, closingDate) {
            return cost.templateId == 4 ? cost.monthsToBePaid * $filter('impRound')(cost.amountPerMonth, 2) : commonService.compareDatesCustom(closingDate, cost.closingDateEnd, enums.timeInterval.days) * $filter('impRound')(cost.interestRate, 2);
        }
        function updateImpounds(data, wrappedLoan) {
            ...
            for (var i = 0; i < data.costs.length; i++) {
                ...
                if (!matchingCosts.length) {
                    ...
                    if (newCost.hudLineNumber != 1003 || newCost.amountMethod == 3 /* MonthlyMI */)
                    ...
                    switch (oldCost.hudLineNumber) {
                        ...
                        case 1006:
                            ...
                            break;
                        case 1003:
                            if (newCost.amountMethod == 3 /* MonthlyMI */)
        ...
        */
        function calculateAllowableBorrowerPaidClosingCost() {
            return costDetailsSvc.wrappedLoan.ref.closingCost.totals.loanCosts.borrowerTotal + (costDetailsSvc.wrappedLoan.ref.closingCost.totals.otherCosts.borrowerTotal - calculateSectionTotal(5 /* Prepaids */));
        }
    }
})();
//# sourceMappingURL=costdetails.helpers.service.js.map